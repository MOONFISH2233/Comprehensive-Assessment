/**
 * 内置的默认细则：重庆大学国家卓越工程师学院 2026 下学期
 *
 * 数据来源：《国家卓越工程师学院2026下学期学生综合测评奖励加分项目实施细则》
 * 页码标注在每一组上方。
 *
 ...
 *   penalty      整组一屏，扣分项
 *
 * 本文件是「内置的那一份」。用户自己导入或编辑的细则存在 localStorage，
 * 见 catalog-store.js。运行时读的是 store 里当前选中的那份。
 */
...
const GROUPS = [
  ...
  // 细则 p4「（1）科研与创新加分」
  //
  // 注：这里原本是一个带 dachuang/patent/paper 自定义字段的分组，但那些字段
  // 不在通用的数据模型里 —— 分级表单读 group.levels[0] 会直接抛异常。
  // 现在拆成两个标准分组：大创（级别 × 结题成绩）走 grade，其余每项定额走 count。
  {
    id: 'Z-dachuang',
    yu: 'Z',
    title: '大创项目',
    note: '大学生创新创业训练计划项目，按级别和结题成绩计分。完成单位须为重庆大学。',
    cap: null,
    mode: 'grade',
    rankNames: ['优秀', '良好', '合格'],
    levels: [
      { key: 'national', name: '国家级', scores: [6, 5, 4] },
      { key: 'province', name: '省部级', scores: [4, 3, 2] },
      { key: 'school',   name: '校级',   scores: [2, 1, 0.5] }
    ],
    stampNote: STAMP_NOTE
  },

  // 细则 p4「（1）科研与创新加分」中的专利 / 论文 / 种子轮
  {
    id: 'Z-zhuanli',
    yu: 'Z',
    title: '专利 · 论文 · 创业',
    note: '每项定额计分。论文加分需为第一作者（含指导老师为第一作者、学生为第二作者）。' +
          '核心期刊认定参照北京大学图书馆最新版《中文核心期刊要目总览》。',
    cap: null,
    mode: 'count',
    unit: 1,
    items: [
      { id: 'Z-zl-inv',   name: '发明专利', unit: 2 },
      { id: 'Z-zl-util',  name: '实用新型专利', unit: 1 },
      { id: 'Z-lw-sci',   name: '在 SCI / EI / SSCI 收录的期刊上发表论文', unit: 6 },
      { id: 'Z-lw-core',  name: '在 CSSCI / CSCD 收录的期刊或核心期刊上发表论文', unit: 3 },
      { id: 'Z-cy-seed',  name: '明月班项目获种子轮融资', unit: 6 }
    ]
  },
...
/**
 * 一份完整的「细则」。用户自己导入或编辑的细则也是这个形状。
 * 换个学校/学院，本质上就是换这样一份数据。
 */
export const DEFAULT_CATALOG = {
  id: 'zgzy-cqu-2026f',
  name: '国家卓越工程师学院 · 2026 下学期',
  school: '重庆大学',
  college: '国家卓越工程师学院',
  term: '2026 下学期',
  deadline: '2026-09-25 17:59',
  materialRange: '2026.03.02 – 2026.09.05',
  notes: [
    '材料落款日期须在 2026.03.02 – 2026.09.05 之间。',
    '上学期已加分的奖项不得重复申报。经查实重复加分的，按学校有关规定处理，取消本学年评奖评优资格。',
    '奖项级别以落款章为准：落款章为「重庆大学」或「重庆大学党委」的认定为学校级，其余认定为学院级。',
    '活动类照片须本人出镜；劳育加分须提供完整的志愿时长截图。'
  ],
  yu: YU,
  formulaNames: FORMULA_NAMES,
  groups: GROUPS,
  // 细则与学校办法数值不一致处。工具一律取学院细则，但要在界面上提醒学生。
  conflicts: {
